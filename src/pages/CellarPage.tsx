...
const CellarPage: React.FC = () => {
  ...
  const { sentinelRef: loadMoreRef, triggered, reset } = useInfiniteScroll(hasMore);

  // Load next page when sentinel triggers
  useEffect(() => {
    if (triggered) {
          ...
          <>
            ...
            {hasMore && (
              <div ref={loadMoreRef} className="flex justify-center py-8">
                <Body size="caption" colour="ghost" align="centre">
                  Showing {visibleCount} of {filteredInventory.length} wines
                </Body>
              </div>
            )}
          </>
