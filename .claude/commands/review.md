Run a comprehensive review of recent changes using all 6 review agents.
...
Then launch ALL 6 agents in parallel using the Agent tool, passing each one the list of changed files to scope their review:
...
6. **gemini** agent — audits AI pipeline (prompts, personality consistency, query routing, RAG, streaming, cost)

Note: If the changed files don't touch any AI-related code (services/*Service.ts, hooks/useGeminiLive.ts, constants.tsx, greetings.ts, functions/), you may skip the gemini agent and note "No AI changes — skipped" in the summary.
...
| Gemini      | pass/warn/fail/skipped | [1-line summary] |
...
- **skipped**: Agent not relevant to the changed files
